...
export async function fund (signer: Wallet, contractOrAddress: string | Contract , amountEth = '0.001'): Promise<void> {
  let address: string
...
export async function getBalance (signer: Wallet, address: string): Promise<number> {
...
export function createAccountOwner (signer: Wallet): Wallet {
  const privateKey = keccak256(Buffer.from(arrayify(BigNumber.from(++counter))))
  return new ethers.Wallet(privateKey, signer.provider)
  // return new ethers.Wallet('0x'.padEnd(66, privkeyBase), ethers.provider);
}

export function createAddress (signer: Wallet): string {
  return createAccountOwner(signer).address
}
...
export async function calcGasUsage (signer: Wallet, rcpt: ContractReceipt, entryPoint: EntryPoint, beneficiaryAddress?: string): Promise<{ actualGasCost: BigNumberish }> {
  ...
  console.log('\t== calculated gasUsed (paid to beneficiary)=', actualGasUsed.toString())
  const tx = await signer.provider.getTransaction(rcpt.transactionHash)
...
/* export async function deployEntryPoint (account:Wallet): Promise<EntryPoint> {
  const create2factory = new Create2Factory(account.provider, account)
  create2factory.deployFactory();
  const epf = new EntryPoint__factory(account)
  ...
  return EntryPoint__factory.connect(addr, account)
} */

export async function isDeployed (signer: Wallet, addr: string): Promise<boolean> {
